...
const { kiba, kiba1,cmd, jsonformat, botpic, TelegraPh, RandomXP, Config, tlang, warndb, sleep,getAdmin, prefix } = require('../lib')
...
cmd({
        pattern: "support",
        desc: "Sends KIBALANGA userbot group link.",
        category: "group",
    },
    async(Void, citel, text) => {
        ...
        await Void.sendMessage(`${citel.sender}`, {
            image: log0,
            caption: `*Group Name: Kibalanga-Support*\n*Group Link:*https://chat.whatsapp.com/B6JB9JD6MnhEyhUL4EIN7G `,
        });
...
\n\n`
            for (let i = 0; i < fetchlb.length; i++) {
                ...
                let data = await kiba1.findOne({ id: fetchlb[i].userID })
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            memek = await Void.sendImageAsSticker(citel.chat, meme, citel, { packname: citel.pushName, author: 'kibalanga' })
...
cmd({ on: "text" }, async(Void, citel) => {
    ...
    if (hasLeveledUp) {
        const kiba1 = await Levels.fetch(citel.sender, "RandomXP");
        const lvpoints = kiba1.level;
        var role = "GOD";
        if (lvpoints <= 2) {
            var role = "🏳Citizen";
        } else if (lvpoints <= 4) {
...
║ *👤Name*: ${citel.pushName}
║ *🎐Level*: ${kiba1.level}🍭
║ *🛑Exp*: ${kiba1.xp} / ${Levels.xpFor(kida1.level + 1)}
║ *📍Role*: *${role}*
║ *Enjoy🥳*
╚════════════╝
`,
