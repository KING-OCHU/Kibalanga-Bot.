 ...
 * @modified by : @SAM-OCHU <https://github.com/SAM-OCHU>
 * @description : Kibalanga,A Multi-functional whatsapp bot.
 * @version 0.0.6
 **/

 const { kiba,kiba1,cmd, getBuffer, tlang, prefix } = require('../lib')
...
async(Void, citel, text,{ isCreator }) => {
    ...
    switch (value) {
        ...
        return await Void.sendButtonText(citel.chat, buttons, `*1000 💎diamond storage has been added in ${pushname} bank*`, `Kibalanga-Economy
Version: 0.0.6`, citel);
        ...
        return await Void.sendButtonText(citel.chat, buttonss, `*100000 💎diamond storage has been added in ${citel.pushName} bank*`, `Kibalanga-Economy
Version: 0.0.6`, citel);
        ...
        return await Void.sendButtonText(citel.chat, buttonst, `*10000000 💎diamond storage has been added in ${citel.pushName}\'s bank*`, `Kibalanga-Economy
Version: 0.0.6`, citel);
...
default:
 await citel.reply('*What are you trying to do📉*.')
    ...
    return await Void.sendButtonText(citel.chat, buttons, `Successfully 💰Deposited 💎${deposit.amount} to your bank.Upgrade your bank capacity to add more money📈.`, `Kibalanga-Economy
Version: 0.0.6`, citel);
    }
)
...
async(Void, citel, text,{ isCreator }) => {
    ...
    return await Void.sendButtonText(citel.chat, buttons, `*📠 Transaction successful of ${value[0]} 💰*`, `Kibalanga-Economy
Version: 0.0.6`, citel);
...
Version: 0.0.6`, citel);
    ...
    switch (random) {
        case 'ran':
              ...
              break
        case 'rob':
          const deduct1 = await eco.deduct(user2, kibalanga, balance2.wallet);
          const add2 = eco.give(user1, kibalanga, balance2.wallet);
...
default:
 ...
 ///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
   ...
   let sticker = new Sticker(media, {
    ...
    return await Void.sendButtonText(citel.chat, buttons, `*📈 You won 💎${twice}*`, `Kibalanga-Economy
Version: 0.0.6`, citel);

        }
        else{
           let deduct = await eco.deduct(user, Kibalanga, texts[0]);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
    ...
    return await Void.sendButtonText(citel.chat, buttons,`*📉 You lost 💎${texts[0]}*`, `Kibalanga-Economy
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
            ...
            const Kibalanga = "Kibalanga"
            const k = 100
            const balance1  = await eco.balance(user, kibalanga)
...
const balance = await eco.balance(citel.sender, kibalanga)
console.log(balance.wallet)
if(value<=balance.wallet){
    const deduff = Math.floor(Math.random() * value)
    if ((f1 !== f2) && f2 !== f3){
        const deduct1 = await eco.deduct(user, kibalanga, deduff);
        return citel.reply(`${mess1}\n\n*Big Lose -->* _💎${deduff}_`)
     }
     else if ((f1 == f2) && f2 == f3){
        const give1 = await eco.give(user, kibalanga, deduff/2);
        return citel.reply(`${mess2}\n*_Little Jackpot -->* _💎${deduff/2}_`)
     }
     else if ((f1 == f2) && f2 !== f3){
        const give2 = await eco.give(user, kibalanga, deduff);
        return citel.reply(`${mess3}\n*Small Win -->* _💎${deduff}_`)
     }
     else if ((f1 !== f2) && f1 == f3){
        const deduct2 = await eco.deduct(user, kibalanga, deduff);
        return citel.reply(`${mess5}\n\n*Small Lose -->* _💎${deduff}_`)
     }
     else if ((f1 !== f2) && f2 == f3){
        const give4 = eco.give(user, kibalanga, deduff);
        return citel.reply(`${mess3}\n\n*Small Win -->* _💎${deduff}_`)
     }
     else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
        const give5 = eco.give(user, kibalanga, deduff*20);
    ...
    return citel.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
            ...
            if ((f1 !== f2) && f2 !== f3){
               const deduct1 = await eco.deduct(user, kibalanga, 50);
                      citel.reply(`${mess1}\n\n*Big Lose -->* _💎50_`)
            }
            else if ((f1 == f2) && f2 == f3){
               const give1 = await eco.give(user, kibalanga, 100);
                     citel.reply(`${mess2}\n*_Little Jackpot -->* _💎100_`)
            }
            else if ((f1 == f2) && f2 !== f3){
               const give2 = await eco.give(user, kibalanga, 20);
                     citel.reply(`${mess3}\n*Small Win -->* _💎20_`)
            }
            else if ((f1 !== f2) && f1 == f3){
               const deduct2 = await eco.deduct(user, kibalanga, 20);
                     citel.reply(`${mess5}\n\n*Small Lose -->* _💎20_`)
            }
            else if ((f1 !== f2) && f2 == f3){
               const give4 = eco.give(user, kibalanga, 20);
                     citel.reply(`${mess3}\n\n*Small Win -->* _💎20_`)
            }
            else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
               const give5 = eco.give(user, kibalanga, 1000);
                    citel.reply(`${mess4}\n\n_🎊 JackPot --> _💎1000_`)
            }
            else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "Kibalanga")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "kibalanga", deduff*10);
    ...
    return await Void.sendButtonText(citel.chat, buttons,str+`You got ${deduff*10} in your wallet.`, `Kibalanga-Economy
Version: 0.0.6`, citel);

    } else {
    const deduff = Math.floor(Math.random() * 300)
    const deduct1 = await eco.deduct(citel.sender, "kibalanga", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
    let buttons = [{
        buttonId: `${prefix}slot`,
...
return await Void.sendButtonText(citel.chat, buttons,str+` ${deduff}.`, `Kibalanga-Economy
Version: 0.0.6`, citel);
}
}
)