...
setTimeout(() => {
	...
	try{
		...
		console.log('Could not connect with Mongodb.\nPlease visit https://kibalangabot.tech/wiki')
	}
    }
    main()
    //========================================================================================================================================
    ...
    async function syncdb() {
        ...
        global.log0 = fs.readFileSync(__dirname + "/assets/SGN_12_17_2022_1671264530426_1.pgn"); //ur logo pic
	...
	let packnamee = options.packname || 'Kibalanga'
	let authore = options.author || 'SAM-OCHU'
	...
	return await Void.sendMessage(citel.chat,{sticker:{url:data}},{quoted:citel})
                ...
                let supportdev = (citel.sender === '255678101729@s.whatsapp.net') || (citel.sender === '255678101729@s.whatsapp.net')
                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            if (zx < 25) {
                                ...
                                var textuser = budy
                                let fetchtext = await fetchk(`http://api.brainshop.ai/get?bid=171008&key=1E8uiNV3mGFm3a18&uid=[uid]&msg=[msg][${diffuser}]&msg=[${textuser}]`);
                            ...
                            if (zx < 20) {
                                var diffuser = citel.sender.split("@")[0];
                                let fetchk = require("node-fetch");
                                let fetchtext = await fetchk(` http://api.brainshop.ai/get?bid=171008&key=1E8uiNV3mGFm3a18&uid=[uid]&msg=[msg][${diffuser}]&msg=[${citel.text}]`);
                                let json = await fetchtext.json();
                                let { cnt } = json;
				    console.log(cnt)
                                citel.reply(cnt);
                        ...
                        if (pattern.test(chab)) {
                            ...
                            try {
                                ...
                                const timesam = moment(moment())
                                    .format('HH:mm:ss')
                                moment.tz.setDefault('AFRICA/TANGA')
                                    .locale('id')
                                await new warndb({
...
Its been ${clockString(new Date() - afkTime)}\n\nThanks\n*Powered by SAM TECHNOLOGY BRAND
...
<html>
  ...
  <body>
    <section>
      Hello! this is KIBALANGA BOT from SAM-OCHU.
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`Kibalanga Server listening on port http://localhost:${port}!`));
