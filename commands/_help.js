...
const Kibalanga = require('../lib/commands')
    //---------------------------------------------------------------------------
Kibalanga.cmd({
            pattern: "help",
            alias: ["menu"],
        ...
        async(Void, citel, text) => {
            ...
            if (text.split(" ")[0]) {
                ...
                const time = moment(moment())
                    .format('HH:mm:ss')
                moment.tz.setDefault('AFRICA/TZ')
                    .locale('id')
                const date = moment.tz('AFRICA/TZ').format('DD/MM/YYYY')
                let total = await sck1.countDocuments()
                let str = `╭────〔 SAM ` + fancytext(Config.ownername.split(' ')[0], 58) + ` 〕─────⊷\n`
                str +=
                    '```' + `│ ╭──────────────
│ │ User:- ${citel.pushName}
│ │ Theme:- ${tlang().title}
